    ...
    <thead class="thead-dark">
        ...
        @foreach ($users as $key=>$user) 
            ...
            <td><a href="{{ route('company.destroy',$user->id) }}" class="btn btn-sm btn-outline-danger py-0" style="font-size: 0.8em;" id="deleteCompany" data-id="{{ $user->id }}">
                ...
                <td><a href="{{ route('delete.user',$user->id) }}" class="btn btn-sm btn-outline-danger py-0" style="font-size: 0.8em;" id="delete" data-id="{{ $user->id }}">
                Delete
                </a></td>

                <td>   <form action="{{ route('kamar_theresia.destroy', $tere->id) }}" method="POST">
    @csrf
    @method('delete')
